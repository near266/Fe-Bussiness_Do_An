import { ApproveStatusId } from '@/interfaces/models/IRecruitment';
...
const ListPost = () => {
  ...
  const columns = [
    {
      title: 'NỘI DUNG TIN DĂNG',
      dataIndex: 'title',
    },
    {
      title: 'SL ỨNG TUYỂN',
      dataIndex: 'total_cv',
      render: (_, record) => <p>{record.total_cv ?? 0}</p>,
    },
    {
      title: 'LƯỢT XEM',
      dataIndex: 'total_view',
      render: (_, record) => <p>{record.total_view ?? 0}</p>,
    },
    {
      title: 'THỜI GIAN',
      dataIndex: 'created_date',
      render: (_, record) => <p>{moment(record.created_date).format('DD/MM/YYYY')}</p>,
    },
    {
      title: 'THAO TÁC',
      ...
      render: (_, record) => {
        return (
          <div className="flex justify-around">
            ...
            >
              <Image src={SrcIcons.editActionIcon} height={24} width={24} alt="icon" />
            </IconButton>
            <Popconfirm
              placement="leftTop"
      ...
      render: (_, record) => {
        return (
          <span className="text-[#696974] flex justify-start items-center gap-2">
            {ApproveStatusIdSVG[record.approve_status_id]}
            {record.approve_status_id === ApproveStatusId.APPROVED ? (
              ApproveStatusId.VN_APPROVED
            ) : record.approve_status_id === ApproveStatusId.REJECTED ? (
              <Tooltip content={`Lý do từ chối: ${record?.reason_of_rejection ?? ''}`}>
