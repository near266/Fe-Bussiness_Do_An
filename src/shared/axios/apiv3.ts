...
const apiEnterprise: AxiosInstance = axios.create({
  baseURL: process.env.NEXT_PUBLIC_ENTERPRISE_API_URL,
  timeout: 10000,
  headers: {
...
apiEnterprise.interceptors.request.use(
  (config) => {
...
apiEnterprise.interceptors.response.use(
  (response) => {
    return response;
  },
  async (error) => {
    ...
    if (isRefreshing) {
      return new Promise((resolve, reject) => {
        ...
        .then((token) => {
          return apiEnterprise(originalRequest);
        })
        .catch((err) => {
    ...
    if (res.code === SV_RES_STATUS_CODE.success) {
      ...
      return Promise.resolve(apiEnterprise(originalRequest));
    }
