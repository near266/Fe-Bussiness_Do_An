...
export function getAccessTokenFromServerSide(cookies) {
  return getByName(cookies, process.env.NEXT_PUBLIC_ACCESS_TOKEN_NAME);
}

export function redirectToAuthenticate() {
...
export function showAntFormError(
  errors: {
    name: (string | number)[];
    errors: string[];
    warnings: string[];
  ...
  const errorList = errors
    ...
    .filter((item) => item);
  if (errorList.length) {
    return message.error(errorList.join(' -\t '), 4);
  }
  return true;
}
